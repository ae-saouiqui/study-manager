...
let Rapport=getForm('add-rapport');
Rapport.addEventListener('submit',(event)=>{
    event.preventDefault();
    var title=Rapport.children[0].value;
    var prof=Rapport.children[1].value;
    var fichier=Rapport.children[2].files[0];
    var etudiant=Rapport.children[1].dataset.etudId;
    let data=new FormData();
    data.append('title',title);
    data.append('prof',prof);
    data.append('file',fichier);
    data.append('etudiant',etudiant);
    let request=new XMLHttpRequest();
    request.open("POST","/index.php?action=addRapport",true);
    request.send(data);
